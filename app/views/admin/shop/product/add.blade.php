    ...
    <div class="col-md-7">
        <div class="box box-primary">
            <div class="box-header">
                <h3 class="box-title">Добавление товара</h3>
            </div>
            {{ Form::open() }}
                <div class="box-body">
                    @if($errors->has())
                    ...
                    <div class="form-group {{ $errors->first('stock') ? 'has-error' : '' }}">
                        ...
                        {{ Form::text('stock', '', ['class' => 'form-control', 'placeholder' => 'Количество на складе']) }}
                    </div>
                    <div class="row">
                        <div class="col-lg-8">
                            {{ Form::label('price', 'Цена')}}
                            <div class="{{ $errors->first('price') ? 'has-error' : '' }}">
                                {{ Form::text('price', '', ['class' => 'form-control', 'placeholder' => 'Цена']) }}
                            </div>
                        </div>
                        <div class="col-lg-4">
                            {{ Form::label('currency', 'Валюта') }}
                            <div class="{{ $errors->first('currency') ? 'has-error' : '' }}">
                            {{ Form::select('currency_id', [1 => 'Гривна', 2 => 'Доллар'], 1, ['class' => 'form-control']) }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group {{ $errors->first('content') ? 'has-error' : '' }}">
                        ...
                        {{ Form::textarea('content', '', ['class' => 'form-control', 'placeholder' => 'Содержание', 'rows' => 3]) }}
                    </div>
                                    ...
                                    @if (Session::get('images'))
                                        @foreach (explode( ',' , Session::get('images')) as $i => $image)
                                            <tr  class="ui-state-default">
                                                ...
                                                <td><img src="/uploads/images/product/{{ $image }}_small.png?{{time()}}"></td>
...
<script type="text/javascript">
    $(function() {
        ...
        $('#name').friendurl({id : 'link', transliterate: true});
