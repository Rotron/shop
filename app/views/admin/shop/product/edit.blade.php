            ...
            {{ Form::open() }}
                <div class="box-body">
                    ...
                    <div class="form-group {{ $errors->first('stock') ? 'has-error' : '' }}">
                        {{ Form::label('stock', 'Количество на складе')}}
                        {{ Form::text('stock', $item->stock, ['class' => 'form-control', 'placeholder' => 'Количество на складе']) }}
                    </div>
                    <div class="row">
                        <div class="col-lg-8">
                            {{ Form::label('price', 'Цена')}}
                            <div class="{{ $errors->first('price') ? 'has-error' : '' }}">
                                {{ Form::text('price', $item->price, ['class' => 'form-control', 'placeholder' => 'Цена']) }}
                            </div>
                        </div>
                        <div class="col-lg-4">
                            {{ Form::label('currency', 'Валюта') }}
                            <div class="{{ $errors->first('currency') ? 'has-error' : '' }}">
                            {{ Form::select('currency_id', [1 => 'Гривна', 2 => 'Доллар'], $item->currency_id, ['class' => 'form-control']) }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group {{ $errors->first('description') ? 'has-error' : '' }}">
                        ...
                        {{ Form::textarea('description', $item->description, ['class' => 'form-control', 'placeholder' => 'Описание', 'rows' => 3]) }}
                    </div>
                    <div class="form-group {{ $errors->first('content') ? 'has-error' : '' }}">
                        {{ Form::label('content', 'Содержание')}}
                        {{ Form::textarea('content', $item->content, ['class' => 'form-control', 'placeholder' => 'Содержание', 'rows' => 3]) }}
                    </div>
                    ...
                    <div class="form-group hide {{ $errors->first('images') ? 'has-error' : '' }}">
                        ...
                        {{ Form::text('images', $item->images, ['class' => 'form-control', 'placeholder' => 'Изображения', 'rows' => 3]) }}
                    </div>
                    <div class="form-group">
    ...
    <div class="col-md-5">
        <div class="box box-warning">
            ...
            <form id = "fileupload">
                ...
                <input type="text" name="number" value="{{ $item->images }}" id="number" class="hide">
            </form>
                      ...
                      <div id="template" class="file-row">
                        <table class="table table-striped table-images" id="table-images">
                            ...
                            <tbody id="sortable">
                                    ...
                                    @if (Session::get('images'))
                                        @foreach (explode( ',' , Session::get('images')) as $i => $image)
                                            <tr  class="ui-state-default">
                                                ...
                                                <td><img src="/uploads/images/product/{{ $image }}_thumb.png"></td>
                                ...
                                @else
                                    @if ($item->images)
                                        @foreach (explode( ',' , $item->images) as $i => $image)
                                            <tr  class="ui-state-default">
                                                <td class="number">{{ $i+1 }}</td>
                                                <td><img src="/uploads/images/product/{{ $item->id }}_{{ $i+1 }}_thumb.png?{{time()}}"></td>
                                                <td><button  data-img="{{ $image }}" data-id="{{ $i+1 }}" class="btn btn-danger delete margin"><i class="glyphicon glyphicon-trash"></i> Удалить</button>
                                                 </td>
                                            </tr>
                                        @endforeach
                                    @endif
        ...
        $('#image').fileupload({
            dataType: 'json',
                ...
                var buttonDelete = $('<button  data-img="' + data.result.tmp + '" data-id="' + rowCount + '" class="btn btn-danger delete margin"><i class="glyphicon glyphicon-trash"></i> Удалить</button>').click(function () {
                    ...
                    $.post( "/admin/product/deleteImage", { img: $(this).data('img') })
                });
